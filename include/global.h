...
#define IN_CLASS_ROOT(xo)  ((xo)->key == CH_END - 1)
...
#define FN_BMW		"bmw"		/* 叉滴記錄 Binary Message Write */
#define FN_AMW		"amw"		/* 叉滴記錄 ASCII Message Write */
...
#define FN_GEMNOTE	"gemnote"	/* 進精華區畫面 */
#define FN_POSTLAW    	"postlaw"       /* 發文綱領 */
#define FN_EXPLAIN      "explain"	/* 看板說明 */
#define FN_PREFIX	"prefix"	/* hrs.080227: 發文類別改用定義 */
#define FN_SAMPLE	"SamplePost"	/* hrs.080227: 文章範本 */
#define FN_BAD          "badlist"       /* hrs.080330: 看板水桶名單 */
#define FN_RSSCONF	"rss.conf"	/* hrs.090408: RSS設定 */
...
#define FN_ETC_SLOGAN	"etc/slogan"		/* 進站footer */

#define FN_ETC_LOVELETTER "etc/loveletter"	/* 情書產生器文庫 */

#define FN_RSS_FOLDER	"etc/rss/"		/* 全站RSS設定的目錄 */

#define FN_RSS_LIST    FN_RSS_FOLDER "feed.list"/* RSS feed list */
...
#define	BN_BUGREPORT	"XDBug"		/* Bug 回報板 */

#ifdef HAVE_ALLPOST
#   define BN_ALLPOST	    "ALLPOST"	/* 跨板Local新文章備份處 */
#   define BN_ALLHIDPOST    "ALLHIDPOST"/* 隱板Local新文章備份處 */
#endif

#ifdef HAVE_LUCKYPOST
#   define BN_LUCKYPOST	    "LuckyPost"	/* 幸運文章備份處 */
#endif
...
#define STR_NEW		"new"
#ifdef  BMW_ROBOT
#   define STR_ROBOT	"NPC"
#endif
#define STR_OPERATORS	"站長/群組長"

#define STR_ANONYMOUS	"叉滴路人甲"		/* 要短於 IDLEN 個字 */

#ifdef LOGINASGUEST
#define STR_LOGINPROMPT1 "，或輸入 " STR_GUEST" 參觀"
#else
#define STR_LOGINPROMPT1 "，本站不開放使用 " STR_GUEST " 參觀"
#endif

#ifdef LOGINASNEW
#define STR_LOGINPROMPT2 "，註冊新帳號請輸入 " STR_NEW " "
#else
#define STR_LOGINPROMPT2 ""
#endif
...
#define ERR_PASSWD	"密碼輸入錯誤，請重新輸入..."
...
#define	MSG_LL		ANSI_COLOR(32) "[群組名單]" ANSI_RESET "\n"
#define MSG_DATA_CLOAK	"<資料保密>\n"

#define MSG_CHKDATA	"★ 資料整理稽核中，請稍候 " ANSI_COLOR(5) "..." ANSI_RESET
#define MSG_QUITGAME	"不玩了啊？下次再來哦！ ^_^"

#define MSG_CHAT_ULIST	\
ANSI_COLOR(7) " 使用者代號    目前狀態  │ 使用者代號    目前狀態  │ 使用者代號    目前狀態 " ANSI_RESET
...
VAR int ibno;			/* hrs.080321:ibno用來記錄cursor的bno或currbno，info.c用 */

VAR uschar isgetdata;		/* hrs.080218:正在b_lines上getdata(vget, vans) */
VAR uschar isSu;		/* hrs.101101:被su的使用者 */
/* filename */
...
VAR char *fn_bad	INI(FN_BAD);
VAR char *fn_plans	INI(FN_PLANS);
VAR char *fn_note	INI(FN_NOTE);
VAR char *fn_gemnote	INI(FN_GEMNOTE);
VAR char *fn_explain    INI(FN_EXPLAIN);
VAR char *fn_sample	INI(FN_SAMPLE);
...
VAR char *str_operators	INI(STR_OPERATORS);
#ifdef  BMW_ROBOT
VAR char *str_robot	INI(STR_ROBOT);
#endif

VAR char *str_ransi	INI(ANSI_RESET);
