...
#define UFO_MOTD	BFLAG(6)	/* 簡化離站畫面 */
...
#define UFO_BMWDISPLAY	BFLAG(13)	/* itoc.010315: 叉滴回顧介面 */
#define UFO_SAVEMEMO    BFLAG(14)	/* lkchu.990510: 自動存信件備份等紀錄 */
#define UFO_NTLOG       BFLAG(15)	/* lkchu.990510: 不存聊天紀錄 */

#define UFO_NOSIGN	BFLAG(16)	/* itoc.000320: 不使用簽名檔 */
#define UFO_UNREADCOLOR	BFLAG(17)	/* hrs.100113: 使用顏色標示事後未讀文章 */

#define UFO_BRDEXP	BFLAG(18)	/* hrs.080102.todo:看板說明 */
#define UFO_JUMPBRD	BFLAG(19)	/* itoc.020122: 自動跳去下一個未讀看板 */
#define UFO_ZHC         BFLAG(20)       /* hightman.060504: 全型字偵測 */
#define UFO_IDLELABEL	BFLAG(21)	/* hrs.080102: 使用者名單看閒置時間 */
...
#define UFO_DEFAULT_NEW		(UFO_BRDNOTE | UFO_MOTD | UFO_BMWDISPLAY | UFO_BRDEXP)
#define UFO_DEFAULT_GUEST	(UFO_MOVIE | UFO_BRDNOTE | UFO_QUIET | UFO_NOALOHA | UFO_NTLOG | UFO_NOSIGN | UFO_BRDEXP)
...
#define NUMUFOS_USER	23	/* 一般使用者 可以用前 23 個 ufo */
...
char *ufo_tbl[NUMUFOS] =
{
  ...
  "呼叫器          (好友/所有)",	/* UFO_PAGER */
#ifdef HAVE_NOBROAD
  "不接收廣播      (拒收/接收)",	/* UFO_RCVER */
#else
  "保留",
#endif
  "打開防水        (防水/開放)",	/* UFO_QUITE */
...
#ifdef BMW_DISPLAY
  "叉滴回顧介面    (完整/上次)",	/* UFO_BMWDISPLAY */
#else
  "保留",
#endif
  "預設儲存信件備份(儲存/不存)",	/* UFO_SAVEMEMO */
  "刪除聊天紀錄    (刪除/問我)",	/* UFO_NTLOG */

  "不使用簽名檔    (不用/問我)",	/* UFO_NOSIGN */
  "新推文未讀使用  (顏色/符號)",	/* UFO_UNREADCOLOR */			
  "看板說明        (使用/不用)",	/* hrs: UFO_BRDEXP */
  ...
  "看閒置時間/標記 (閒置/標記)",	/* UFO_IDLELABEL */
  "保留",	/* hrs.080321: UFO_UNREADMARK */
  "保留",
