  ...
  textline *this;
{
  ...
  {    
    ...
    show_sign:
    if (!(cuser.ufo & UFO_NOSIGN))	/* hrs.080325: 改用NOSIGN */
      show_sign('0' + topsig);          /* itoc.000319: 顯示簽名檔的內容 */

    msg[42] = op = cuser.signature + '0';
  ...
  int modify;			/* 1:修改 0:原文 */
{
	...
	str_ncpy(ipaddr, "127.0.0.1", sizeof(ipaddr));
    else
#endif
    str_ncpy(ipaddr, (char *)str_ip(cutmp->in_addr), sizeof(ipaddr));
    ...
    {
      /* 再比對 ip */
      if (!belong_list(FN_ETC_HOST, ipaddr, fqdn))
      {
  ...
  if (!modify)
  {
    fprintf(fp, EDIT_BANNER, 
      ...
      cuser.userid, 
#ifdef HAVE_ANONYMOUS
      (curredit & EDIT_ANONYMOUS) ? "叉滴路人的家" : 
#endif
       ipaddr,
#ifdef HAVE_ANONYMOUS
  ...
  {
	fprintf(fp, MODIFY_BANNER, cuser.userid, ipaddr, Now());
  }
}
...
#  endif
  ...
  if (ve_op < 0)			/* itoc.010301: 新增 ve_op = -1 不能儲存 */
    ...
    menu = menu2;
  else if (curredit & EDIT_OUTGO && !(brd->battr & BRD_LOCAL))	
    /* 轉信板發文(hrs.080323: 新增預設存站內信看板的判斷) */
    menu = menu3;
  else
    menu = menu4;
	...
	break;
	  ...
	  char datebuf[3][3], i, genbuf[6];
		...
		if (!vget(b_lines, 0, "倒數計時 - 年(2000 ~ 2099): 20", datebuf[0], 3, DOECHO))
		    break;
		if (atoi(datebuf[0]) < 0) break;
		if (!vget(b_lines, 0, "倒數計時 - 月(1 ~ 12): ", datebuf[1], 3, DOECHO))
		    break;
		if (atoi(datebuf[1]) > 12 || atoi(datebuf[1]) < 1) break;
		if (!vget(b_lines, 0, "倒數計時 - 日(1 ~ 31): ", datebuf[2], 3, DOECHO))
		    break;
		if (atoi(datebuf[2]) > 31 || atoi(datebuf[2]) < 1) break;
		sprintf(genbuf, "%02d%02d%02d", atoi(datebuf[0]), atoi(datebuf[1]), atoi(datebuf[2]));
