  ...
  int control;
{
  ...
  if (control)
  sprintf(buf, FOOTER_MORE2,(lino - 2) / PAGE_SCROLL + 1, ((foff - fimage) * 100) / fsize);
  else
...
#ifdef SLIDE_SHOW
static int slideshow;		/* !=0: 有播放; ==0 沒播放 */
static struct timeval slide;
...
more_slideshow()
{
  ...
  if (!slideshow)
  {
    ...
    if (ch == '@' || ch == 'p')
    {
      vget(b_lines, 0, "播放請輸入速度(秒):", buf, 8, DOECHO);
      if (!atof(buf))  return ch;

      slide.tv_sec = atoi(buf);
      if (((atof(buf) - atoi(buf)) * 1000000 < 100000 ) && slide.tv_sec == 0)
	  slide.tv_usec = 100000;
      else 
	  slide.tv_usec = (atof(buf) - atoi(buf)) * 1000000;
      /* hrs.080128: 限制interval在0.1以上 */	
  ...
  else
  {
    refresh();
    ch = 1;
     if (select(1, (fd_set *) &ch, NULL, NULL, &slide) > 0)	 
    {
      /* 若播放中按任意鍵，則停止播放 */
      slideshow = 0;
      ch = vkey();
