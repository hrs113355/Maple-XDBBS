...
#ifdef MY_FAVORITE
#define MSG_ZONE_SWITCH	ANSI_COLOR(1;33;41) "★ (A)精華 (B)文章 (C)看板 (F)最愛 (M)信件 (U)使用者 (W)叉滴 (Q)離站  "
#else
#define MSG_ZONE_SWITCH	ANSI_COLOR(1;33;41) "★ (A)精華 (B)文章 (C)看板 (M)信件 (U)使用者 (W)叉滴 (Q)離站                  "
#endif
  ...
  XO *xo;
{
  ...
  if (!vget(b_lines, 0, "轉寄位置： ", rcpt, sizeof(rcpt), GCARRY))
    return XO_FOOT;
...
XZ xz[] =
{
  ...
  {NULL, rss_cb, M_MBOARD, FEETER_RSS}		/* XZ_RSS */
};
  ...
  int zone;
{
  ...
  if (!(cuser.ufo & UFO_BRDEXP)||(zone != XZ_CLASS &&zone != XZ_MF))
    return;
  ...
  for (bno = b_lines - 3; bno < b_lines; bno++)
    outl(bno, NULL);
  ...
  if (bno >= 0)
  {
    ...
    if (fp = fopen(buf, "r"))
    {
      for (bno = b_lines - 3; bno < b_lines; bno++)     /* 借用 bno */
      {
...
static void rss_preview(XO *xo, int zone)
{
    ...
    move(b_lines - 2, 0);
    clrtoeol();
    prints("屬性設定: [%s]站際存檔 [%s]發文類別 [%s]下載更新 [%s]停止抓取"  
	    , feed->attr & RSS_OUTGO ? "○" : "╳"
  ...
  int cmd;
{
  ...
  for (;;)
  {
	...
	sysmode = xz[cmd].mode;
     ...
     if (IN_CLASS_ROOT(xo))
        XO_TALL = ((cuser.ufo&UFO_BRDEXP)&&(zone==XZ_CLASS||zone==XZ_MF)||zone==XZ_RSS
            ? b_lines - 14 : b_lines - 10);
     else
        XO_TALL = ((cuser.ufo&UFO_BRDEXP)&&(zone==XZ_CLASS||zone==XZ_MF)||zone==XZ_RSS
            ? b_lines - 7 : b_lines - 3);
#else
	...
	{
	  move((IN_CLASS_ROOT(xo) ? 10 : 3) + cmd - num, 0);
          outs("  ");
    ...
    {
      num = (IN_CLASS_ROOT(xo) ? 10 : 3) + pos - xo->top;
      move(num, 0);
    ...
    if (cmd == KEY_LEFT || cmd == 'e')
    {
      ...
      XO_TALL = b_lines - 7;
      if (zone == XZ_MBOX) /* hrs.080323:離開信箱時檢查有無爆容量 */
	  m_overquota(1);
      return;
    }
  ...
  int zone;				/* 傳入所在 XZ_ZONE，若傳入 0，表示不在 xover() 中 */
{
  ...
  if (cmd == zone || cmd == XO_FOOT)		/* 和目前所在 zone 一樣，或取消 */
  {
    z_status--;
