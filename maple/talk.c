...
void
showplans(char * userid, int tall)
{
  int i;
  FILE *fp;
  ...
  if (fp = fopen(buf, "r"))
  {
    for(i = 0; i < tall && fgets(buf, sizeof(buf), fp);i++)
      outx(buf);
    fclose(fp);
  }
  else
   outs("\n\n\n\n                    \033[1;30m 他現在沒有名片:(\033[m");
}
  ...
  ACCT *acct;
{
  ...
  up = utmp_find(userno);
  rich = acct->money >= 50000 ? (acct->money >= 1000000 ? 3 : 2) : (acct->money >= 10000 ? 1 : 0);

  prints(ANSI_COLOR(1;33) "[帳    號]"
	 ANSI_COLOR(1;37) " %-12s "
	 ANSI_COLOR(1;33) "[暱    稱]"
	 ANSI_COLOR(1;37) " %-40.40s\n"
          ,userid, acct->username);

  prints(ANSI_COLOR(1;33) "[上    站]"
	 ANSI_COLOR(1;37) " %5d 次     "
	 ANSI_COLOR(1;33) "[文    章]"
	 ANSI_COLOR(1;37) " %5d 篇\n"
          ,acct->numlogins, acct->numposts);

  prints(ANSI_COLOR(1;33) "[認    證]"
	 ANSI_COLOR(1;37) " %s通過認證 "
	 ANSI_COLOR(1;33) "[動    態]"
	 ANSI_COLOR(1;37) " %-40.40s\n"
          ,acct->userlevel & PERM_VALID ? "已經" : "尚未",
          (up && can_see(cutmp, up)) ? bmode(up, 1) : "不在站上");

  prints(ANSI_COLOR(1;33) "[信    箱]"
         ANSI_COLOR(1;37) " %s     "
         ANSI_COLOR(1;33) "[財    產]"
	 ANSI_COLOR(1;37) " %s "
  ,(m_query(userid) & STATUS_BIFF) ? "有新信件" : "都看過了", fortune[rich]);

  if (HAS_PERM(PERM_ALLACCT) || !strcmp(userid, cuser.userid) 
	  || (up && can_see(cutmp, up) && is_ogood(up)))
      prints("($ %d)\n", acct->money);
  else
      outs("\n");
  
  prints(ANSI_COLOR(1;33) "[人氣指數]"
	 ANSI_COLOR(1;37) " %-12d "
	 ANSI_COLOR(1;33) "[好奇指數]"
	 ANSI_COLOR(1;37) " %d\n"
          ,acct->bequery, acct->toquery);

  prints(ANSI_COLOR(1;33) "[傳叉滴數]"
	 ANSI_COLOR(1;37) " %-12d "
	 ANSI_COLOR(1;33) "[收叉滴數]"
	 ANSI_COLOR(1;37) " %d\n"
          ,acct->numbmwtx, acct->numbmwrx);
  
  prints(ANSI_COLOR(1;33) "[來    源]"
	 ANSI_COLOR(1;37) " (%s) %s\n"
	 ,Btime(&acct->lastlogin), acct->lasthost);

  if (acct->site[0])
  prints(ANSI_COLOR(1;33) "[網    址]"
	 ANSI_COLOR(1;37) " %s \n", acct->site);
