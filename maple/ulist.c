...
#define FTYPE_MYMATE    0x02
#define FTYPE_BOTHGOOD	0x04
#define FTYPE_MYGOOD	0x08
#define FTYPE_OGOOD	0x10
#define FTYPE_NORMAL	0x20
#define FTYPE_MYBAD	0x40

static int mygood_num;		/* 對方設我為好友 */
static int ogood_num;		/* 我設對方為好友 */
static int mybad_num;		/* 我設對方為壞人 */
static int mymate_num;		/* 我設對方為麻吉 */
  ...
  int sysop;
{
  ...
  else 
  {
     ...
     if((up->userlevel & PERM_ROBOT)&&!(cuser.ufo & UFO_IDLELABEL))
	strcpy(buf, "\033[1;34m 機器\033[m");
     else if((up->userlevel & PERM_ART)&& strcmp("SYSOP",up->userid)&&!(cuser.ufo & UFO_IDLELABEL))
	strcpy(buf, "\033[1;35m 美工\033[m");
     else if ((up->userlevel & PERM_ALLADMIN)&&!(cuser.ufo & UFO_IDLELABEL))
	strcpy(buf, "\033[1;33m 站長\033[m");

    else 
#ifdef DETAIL_IDLETIME
    if ((diff = now - up->idle_time) >= 3600) /* hrs:一小時有3600秒 */
      sprintf(buf, "%2dh%02d", diff / 3600, (diff % 3600)/60);
    else if((diff = now - up->idle_time) >= 1)
...
#if 0
    hrs.071219.todo:考生做好之後下面要記得加回來 
      else if (up->ufo & UFO_BMWALARM)
	    strcpy(buf, "\033[33m 考生\033[m");
#endif
  ufo = up->ufo;

  /*         pager 狀態                       */
  /*  W：不接受任何人呼叫，也不接受任何人廣播 */
  /*  *：只接受好友呼叫，且只接受好友廣播     */
  /*  !：只接受好友呼叫，但不接受任何人廣播   */
  /*  -：接受任何人呼叫，但不接受任何人廣播   */
  /*   ：都沒有就是沒有限制啦                 */
...
#  endif
#endif
    up->status & STATUS_BIRTHDAY ? COLOR_BIRTHDAY :
    up->ufo & UFO_SUPERCLOAK ? COLOR_CLOAK :
  ...
  label = cuser.ufo & UFO_IDLELABEL;
  len = (d_cols >> 1);

  if (ftype & FTYPE_MYMATE)
      strcpy(pre, "    " COLOR_MYMATE "★" ANSI_RESET);
  else
      sprintf(pre, "%6d", num);

  prints("%s%c%c%c%s%-13s%-*.*s\033[m%-*.*s%-11.10s\033[1;30m%-4.4s\033[m%s\n",
    pre, ufo & UFO_CLOAK ? ')' : ' ', is_omate(up) ? 'X' : ' ', pager, 
    fcolor, up->userid, 
    label ? (len << 1) + 36 : len + 19,
    label ? (len << 1) + 35 : len + 18,
    label ? up->site : up->username, 
    label ? 0 :d_cols -  len + 17,
    label ? 0 :d_cols - len + 16, 
...
#endif
    up->from : "*", bmode(up, 0), up->feeling, buf);
}
  ...
  UTMP *up;
{
  ...
  if (is_mymate(userno))
    x = FTYPE_MYMATE;
  if (is_mygood(userno))
    return (x | (is_ogood(up) ? FTYPE_BOTHGOOD : FTYPE_MYGOOD));
  return (x | (is_ogood(up) ? FTYPE_OGOOD : FTYPE_NORMAL));
}
  ...
  XO *xo;
{
  ...
  mygood_num = ogood_num = mybad_num = mymate_num = 0;
  ...
  do
  {
	...
	if (userno & (FTYPE_BOTHGOOD | FTYPE_OGOOD))
	  ogood_num++;
	if (userno & FTYPE_MYBAD)
	  mybad_num++;
	if (userno & FTYPE_MYMATE)
	  mymate_num++;
      }
    }
    slot++;
  ...
  XO *xo;
{
  ...
  prints("  排列：[\033[1m%s/%s\033[m] 站上：%d "
    COLOR_MYGOOD " 好友：%d " 
    COLOR_OGOOD " 與我為友：%d" 
    COLOR_MYBAD " 壞人：%d" 
    COLOR_MYMATE " 麻吉：%d" ANSI_RESET, 
    msg_pickup_way[pickup_way], 
    cuser.ufo & UFO_PAL ? "好友" : "全部", 
    total_user, mygood_num, ogood_num, mybad_num, mymate_num);

  prints(NECKER_ULIST, cuser.ufo & UFO_IDLELABEL ? "連結" : "暱稱"
	  ,d_cols >> 1, "", d_cols - (d_cols >> 1) + 4, cuser.ufo & UFO_IDLELABEL ? "" : 
	  pickup_ship ? "友誼" : "故鄉", cuser.ufo & UFO_IDLELABEL ? "閒置" : "標籤");
  ...
  XO *xo;
{
  ...
  if (cuser.userlevel)
  {
	      ...
	      (is_mygood(userno) || is_mybad(userno))) 
    {
      ...
      rec_put(fpath, &pal, sizeof(PAL), xo->pos, NULL);
      pal_cache();                            /* 朋友名單同步 */
      utmp_admset(userno, STATUS_PALDIRTY);
      return XO_HEAD;
    }
...
ulist_broadcast(xo)
  XO *xo;
{
  int num, sysop, soft_broadcast = 0;
  pickup *pp;
  UTMP *up;
  BMW bmw;
  ...
  if (sysop && vans("是否要避開不接收水球及廣播之使用者 [y/N]？ ") == 'y')
      soft_broadcast = 1;
  ...
  if (bmw.caller)	/* bmw_edit() 中回答 Yes 要送出廣播 */
  {
	...
	if (!is_mygood(up->userno))
      ...
      if (soft_broadcast ? can_override(up) > 0 && !(up->ufo & UFO_RCVER) : can_override(up))
      {
  ...
  XO *xo;
{
  ...
  if (HAS_PERM(PERM_ALLADMIN))
  {
      ...
      vget(b_lines, 0, "踢去閒置多久時間以上的使用者？(分)：", buf, 6, DOECHO);
      interval = atoi(buf);
    ...
    {
      up = ulist_pool[pos];
      if (now - up->idle_time >= interval * 60) 
      {
	if (vmsgf("確定要踢除[%s]使用者？ [y/N]", up->userid)!='y')
	    continue;
  ...
  XO *xo;
{
  ...
  strcpy(buf, str = cutmp->site);
  if (vget(b_lines, 0, "請輸入新的連結：", buf, 40, GCARRY))
  {
    if (strcmp(buf, str))
    {
      strcpy(str, buf);
      strcpy(cuser.site, buf);
      if (!(cuser.ufo & UFO_IDLELABEL))
	cuser.ufo ^= UFO_IDLELABEL;
      return ulist_body(xo);
    }
  }
  return XO_FOOT;
...
static KeyFunc ulist_cb[] =
{
  ...
  'l', ulist_recall,		/* 水球回顧 */
  'L', ulist_display,
  'r', ulist_talk,		/* hrs.080108: 應user要求改成talk */
  'q', ulist_query,		/* itoc.020109: 使用者習慣用 q 查詢 */
  'B', ulist_broadcast,
  's', ulist_refresh,		/* refresh status Thor: 應user要求 */
  '.', ulist_refresh,
  'S', ulist_ship,
  'p', ulist_pager,
  'I', ulist_lock,		/* qazq.030719: 使用者選單直接閒置 */
  'K', ulist_kick_idle,		/* hrs.080102: 踢除閒置 */
  'x', ulist_idlelabel,		/* hrs.080102: show idle time instead of label */
  'Q', ulist_all_query,
