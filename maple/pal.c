  ...
  PAL *pal;
{
  ...
  prints("%6d%c%-3s%-14s%s\n",
    num, tag_char(pal->userno), pal->ftype & PAL_BAD ? "Ｘ" :
   pal->ftype & PAL_MATE ? "○" : "", pal->userid, pal->ship);
#endif
}
...
pal_head(xo)
  XO *xo;
{
  char *head[] = {"朋友名單", "群組名單", "看板可見名單", "限制投票名單", "看板水桶名單", "文章可見名單"};

  vs_head(head[xo->key], str_site);
  prints(NECKER_PAL, (xo->key == PALTYPE_PBAD ? "惡 行 惡 狀": "友       誼")
	  ,d_cols, "");
  return pal_body(xo);
}
  ...
  int echo;
{
  ...
  vget(b_lines, 0, key == PALTYPE_PBAD ? "惡行惡狀：" : "友誼："
	  , pal->ship, sizeof(pal->ship), echo);

  if (key == PALTYPE_PAL || key == PALTYPE_LIST || key == PALTYPE_PBAD)
  {      /* 限制投票名單沒有壞人 */
    pal->ftype = vans(key == PALTYPE_PBAD ? "壞人？(水桶名單的壞人進不來這個板) [y/N] " : "壞人？ [y/N] ") == 'y' ? PAL_BAD : 0;
    if (key != PALTYPE_PBAD && vans("麻吉(Y/N)？[N] ") == 'y')
	pal->ftype = PAL_MATE;
  }
  else
    pal->ftype = 0;
  ...
  XO *xo;
{
  ...
  else if (userno > 0)
  {
    ...
    if (xo->key == PALTYPE_BPAL && vans("是否發函通知他(Y/N)？[N] ") == 'y')
    {
      char fpath[64], *title;
      FILE *fp;
