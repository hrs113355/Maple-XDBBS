  ...
  HDR *hdr;	/* 條件 */
{
  ...
  if (hdr->xid && !str_sub(head->owner, hdr->owner))
    return 0;
...
int XoXfilename(XO *xo)
{
    ...
    if (!vget(b_lines, 0, MSG_XYPOST3, xname, 9, DOECHO))
	return XO_FOOT;
    ...
    for (i = 0; i < max; i++)
    {
	int locus = xpostIndex ? xpostIndex[i] : i;
	HDR * hdr = (HDR *) fimage + locus;
#ifdef HAVE_REFUSEMARK
	if (!chkrestrict(hdr))
	    continue;
#endif
...
static int
filter_solve(head, hdr)
  HDR *head;    /* 待測物 */
  HDR *hdr;     /* 條件 */
{
  return (head->xmode & POST_DONE);
}


static int
filter_lock(head, hdr)
      HDR *head;    /* 待測物 */
        HDR *hdr;     /* 條件 */
{
      return ((head->xmode & POST_DONE) && (head->xmode & POST_MARKED));
}

static int
filter_wiki(head, hdr)
      HDR *head;    /* 待測物 */
        HDR *hdr;     /* 條件 */
{
      return (head->xmode & POST_WIKI);
}
...
static int
filter_dellabel(head, hdr)
      HDR *head;    /* 待測物 */
        HDR *hdr;     /* 條件 */
{
      return (head->xmode & POST_DELETE);
}
  ...
  XO *xo;
{
  ...
  sprintf(HintWord, "\033[1;33m所有標記%c的文章\033[m", op);
  HintAuthor[0] = '\0';
  ...
  switch(op)
  {
      ...
      case 's':
          return XoXpost(xo, NULL, 0, INT_MAX, filter_solve);
      case '!':
          return XoXpost(xo, NULL, 0, INT_MAX, filter_lock);
      case 'w':
          return XoXpost(xo, NULL, 0, INT_MAX, filter_wiki);
      case 'x':
          return XoXpost(xo, NULL, 0, INT_MAX, filter_refuse);
      case 't':
          return XoXpost(xo, NULL, 0, INT_MAX, filter_dellabel);
  }
  ...
  XO *xo;
{
  ...
  if (!vget(b_lines, 0, "搜尋評分數大於等於多少的文章？ ", score, 3, DOECHO))
    return XO_FOOT;
  ...
  HintAuthor[0] = '\0';
  sprintf(HintWord, "\033[1;33m評分數大於等於%d的文章\033[m", hdr.score);
