...
int save_hold()
{
    if (cuser.ufo & UFO_SAVEMEMO)
	return (vans("是否自存底稿(Y/N)？[Y] ") != 'n');
    else
	return (vans("是否自存底稿(Y/N)？[N] ") == 'y');
}
...
{
  if (cuser.userlevel && (hold < 0 || save_hold()))
  {
    char buf[256];
  ...
  XO *xo;
{
  ...
  {  
   sprintf(max_sizes, "%4d k",HAS_PERM(PERM_VALID) ? (MAX_VALIDMAILSIZE >> 10)  + (cuser.mquota * 15):
	(MAX_NOVALIDMAILSIZE >> 10) + (cuser.mquota * 15));
   sprintf(max_mails, "%4d 篇", HAS_PERM(PERM_VALID) ? MAX_VALIDMAIL + cuser.mquota:
	   MAX_NOVALIDMAIL + cuser.mquota);
  }


  vs_head("郵件選單", str_site);
  prints(NECKER_MBOX, MboxKB, max_sizes, xo->max, max_mails, d_cols, "");
  ...
  XO *xo;
{
  ...
  for (;;)
  {
    ...
    if ((nmode = more(fpath, FOOTER_MAILER)) == -1)
      break;
...
mbox_gem(xo)		
  XO *xo;
{
char fpath[64], brdname[BNLEN + 1], *str, folder[64];
int bits, level, bno;
BRD *brd;

sprintf(brdname, "P_%-.*s", BNLEN - 2, cuser.userid);
if (ask_board(brdname, BRD_R_BIT | GCARRY, 
	    ANSI_COLOR(1;33)"你想到哪個板的精華區？" ANSI_RESET));

bno = brd_bno(brdname);
brd = bshm->bcache + bno;
brd_fpath(fpath, brd->brdname, fn_dir);
bits = brd_bits[bno];
sprintf(folder, "gem/%s", fpath);
...
m_overflow()
{
  ...
  if (HAS_PERM(PERM_MBOX | PERM_ALLADMIN))
    return 0;
  ...
  if (HAS_PERM(PERM_VALID))
    limit = MAX_VALIDMAIL + cuser.mquota;
  else
    limit = MAX_NOVALIDMAIL + cuser.mquota;
  ...
  *ptr = '\0';
  ...
  if (dirp = opendir(fpath))
  {
    /* 容量幾 byte */
    if (HAS_PERM(PERM_VALID))
      limit = MAX_VALIDMAILSIZE + cuser.mquota * 15 * 1024;
    else
      limit = MAX_NOVALIDMAILSIZE + cuser.mquota * 15 * 1024;
...
int m_overquota(int type)	/* hrs.080323:檢查信箱爆容量 */
/* hrs.080323: type=1 mail內  type=0 bbsd的"第一次"呼叫 */
{
  if (m_overflow())
  {
    film_out(FILM_MAILOVER, -1);
    vmsg("您的信件過多，請購買容量、打包回家或刪除部分信件");

    DL_func("bin/bank.so:b_mquota");
    m_zip();
    if (!HAS_PERM(PERM_VALID))
	u_register();

    if (type) /* hrs:剛上站就show給使用者知道就好了，不要卡住 */
	xover(XZ_MBOX);
  }
}
