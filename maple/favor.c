  ...
  MF *mf;
{
  ...
  else if (mftype & MF_BOARD)
  {
    if ((bno = brd_bno(mf->xname)) >= 0)
      class_item(num, bno, brdpost, 1); /* hrs.100916: mf_item 中 show 出的 item 一定在 favor 中 */
    else
  ...
  else /* if (mftype & MF_CLASS) */	/* LHD.051007: 分類群組 */
  {
    ...
    prints("%6d   " COLOR_INFAVOR "%-13.13s\033[1;3%dm%-5.5s\033[m%s\n",
  ...
  XO *xo;
{
  ...
  if (!(mf->mftype & (MF_FOLDER | MF_LINE | MF_BOARD)))
    return XO_NONE;

  if (mf->mftype & MF_BOARD)
      return mf_brdtitle(xo);
  ...
  XO *xo;
{
  ...
  if (type & MF_BOARD)		/* 看板捷徑 */
  {
    ...
    {
	if (brd_bits[bno] & BRD_L_BIT)  /* hrs.080319: 好友看板 */
	{
  ...
  XO *xo;
{
  ...
  case '1':
    ...
    if (vans("確定要備份我的最愛？可能會覆蓋\掉之前的備份 [y/N]")!='y')
    	return XO_FOOT;
  ...
  case '2':
    ...
    if (vans("確定要還原我的最愛備份？會覆蓋\掉現在的我的最愛 [y/N]")!='y')
       return XO_FOOT;
