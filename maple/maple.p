...
void brd_force(void);
void brd_hiall(void);
void class_item(int num, int bno, int brdpost, int in_favor);
...
int Boards(void);
int class_explain(XO *xo);
inline int in_favor(char * brdname);
...
int m_zip(void);
int m_verify(void);
int mbox_totalsize(char *folder);
usint m_quota(void);
...
void unlink_canseelist(char *brdname, HDR *hdr);
inline int chkrestrict(HDR *hdr);
#endif
void add_post(char *brdname, char *fpath, char *title, char *owner,char *nick, int xmode);
int do_reply(XO *xo, HDR *hdr);
int tag_char(int chrono);
void hdr_outs(HDR *hdr, int cc, int zone);
int post_edit(XO *xo);
...
void allpost_history(XO *xo, HDR *hdr);
int CheckPostPerm(BRD *brd);
...
int t_talk(void);
void talk_rqst(void);
void showplans(char * userid, int tall);
...
/* cosign.c */
int do_cosign(void);
int cosign_reply(XO *xo, HDR *hdr);
int cosign_newbrd(XO *xo);
...
/* record.c */
void log_newbrd(char *bname);
void log_register(ACCT *acct, RFORM *rform, int pass, char * reason);
void ann_newbrd(char *bname);
