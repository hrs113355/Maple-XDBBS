...
int pay_bbsnet(void){
	...
	if(HAS_PERM(PERM_SYSOP))
	    return 1;
	if(vans("雲遊四海一次要 " FEE_BBSNET " 元，確定嗎?：[y/N] ") != 'y')
	    return 0;

	if(cuser.money < atoi(FEE_BBSNET))
	{
		vmsg("餘額不足");
		return 0;
	}
	else cuser.money -= atoi(FEE_BBSNET);
	return 1;
}
#endif

static void
log_bbsnet(conname,stat)
  char *conname;
  int stat;
{
  ...
  sprintf(fpath, "tmp/log_bbsnet.%s", cuser.userid);   /* 暫存檔 */
  if (fp = fopen(fpath, "w"))
  {
    sprintf(title, "[紀錄] BBSNET紀錄(%s)",cuser.userid);
    ...
    /* 文章內容 */
    fprintf(fp, "\n\n使用者 %s 在 %s %s了BBSnet\n\n他連線到 %s\n",
      cuser.userid, Now(),(stat?"使用":"結束"),conname);

    fclose(fp);

    add_post(BN_SECURITY, fpath, title, "[系統紀錄]", "[雲遊仙人]", 0);
    unlink(fpath);
  }
}
static void
...
ptt.twbbs.org:23:PTT
  ...
  while (1)   /* itoc.010824: 連去別的站回來以後還是留在 bbsnet 選單中，
                              可以再連去別的站 */
  {
    ...
    outs("║──────────────────────"
      "───────────────║\n"
      "║ \033[1;31m注意：\033[m當您選擇了站台後，螢幕像"
      "當機一樣不動了，表示現在無法登入對方主機， ║\n"
      "║       請稍候 telnet connect time out 後會自動退"
      "出。                      ║\n"
      "╰═══════════════════════"
      "══════════════╯");
