...
char *usid;
{
  ...
  if (vans("要幫他開個人板(Y/N)？[N] ") != 'y')
    return XEASY;

  clear();
  vs_bar("建立新板");

  sprintf(brdname, "P_%.10s", usid);  /* 板名為 P_userid */
 ...
 for (;;)
  ...
  move(2,0);
  prints("看板分類：個人" );
  vget(3, 0, "看板主題：", buf, BTLEN + 1, DOECHO);
  ...
  str_ncpy(newboard.class, "個人", sizeof(newboard.class));
  ...
  str_ncpy(hdr.title, "[公告] 看板成立", sizeof(hdr.title));
  ...
  int fd;
{
  ...
  static char *reason[] = 
  {
    "請輸入真實姓名", "請詳實填寫申請表", "請詳填住址資料", "請詳填連絡電話", 
    "請詳填服務單位、或學校系級", "請用中文填寫申請單", 
  ...
  while (read(fd, &rform, sizeof(RFORM)) == sizeof(RFORM))
  {
	...
	/* hrs: 在這邊check是因為，到switch才check op == 'd'會有其他非站長設定的情況 */
	if (op == 'd')
	{
	    char reason[80];
	    vget(b_lines, 0, "請輸入刪除原因：", reason, 60, DOECHO);
	...
	if (fout = fdopen(hdr_stamp(folder, 0, &hdr, fpath), "w"))
	{
	  ...
	  str_ncpy(hdr.title, "[退件] 請您重新填寫註冊表單", sizeof(hdr.title));
	  rec_add(folder, &hdr, sizeof(HDR));
	}

        log_register(&acct, &rform, RFORM_NOPASS, buf); /* buf = 理由 */
...
uschar type;	/* 0:國曆 1:農曆 */
FILE *fp;
   
  type = vans("1)國曆日期  2)農曆日期 [Q] ") - '1';
  if (type != 1 && type != 0)
        return XEASY;

  if (!vget(b_lines, 0, "節日在幾月(1 ~ 12): [Q] ", month, 3, DOECHO))
	return XEASY;
  if (atoi(month) < 1 || atoi(month) > 12)
	return XEASY;
  if (!vget(b_lines, 0
    , "節日在幾號(1 ~ 31，或用2A來表示第二個星期日，依此類推...): [Q] ", day, 3, DOECHO))
	return XEASY;
  if (atoi(day) < 0 || atoi(day) > 31 || (day[0] == '0' && !day[1]))
      /* hrs.080224: 考慮05/2A的情況 */
	return XEASY;
  if (day[1] >= 'A' && day[1] <= 'Z')
  ...
  {
      vmsgf("那天是節日 %s ,但沒有設定進站畫面檔名", fename);
      return XEASY;
  }
  ...
  confirm = vmsgf("確認檔名: 那天是 %s ,檔名是 %s 嗎？ [y/N] ", fename, fepath);
  if (confirm != 'y' && confirm != 'Y')
      return XEASY;

  confirm = vansf("節日畫面(%s/%s) (D)刪除 (E)修改 (Q)取消？[E] ", fename, fepath);
