...
static int
vote_readvlist(vitem_t vlist[], VCH * vch, char * dir)
{
    char fpath[64], *fname;
    int fd, count;
    ...
    *fname = 'I';
    ...
    count = read(fd, vlist, MAX_CHOICES * sizeof(vitem_t)) / sizeof(vitem_t);
    close(fd);
  ...
  XO *xo;
{
  VCH vch;
  int fd, item;
  char *dir, *str, fpath[64], title[TTLEN + 1], ipath[64], *vtype;
  ...
  *str = 'I';
  ...
  if (fp = fopen(fpath, "w"))
  {
    vtype = (vch.vgamble == '$' ? "賭盤" : "投票");
    sprintf(title, "[公告] 本板開始%s", vtype);
 
    /* 文章檔頭 */
    fprintf(fp, "%s %s (%s)\n", str_author1, vch.vgamble == '$' ? "[賭神]" : "[投票所]", "系統公告");
    fprintf(fp, "標題: %s\n時間: %s\n\n", title, Now());
 
    /* 文章內容 */
    fprintf(fp, "%s \n\n"ANSI_COLOR(1)"★ 標題： %s\n"ANSI_RESET
	    , title + 7, vch.title);
    fprintf(fp, ANSI_COLOR(1)"★ %s說明：\n\n"ANSI_RESET, vtype);
    f_suck(fp, ipath);
    fprintf(fp, "\n按下大寫 'V' 即可加入%s，請踴躍參加!\n", vtype);

    fprintf(fp, EDIT_BANNER, vch.vgamble == '$' ? "[賭神]" : "[投票所]", "系統公告", "系統公告");

    fclose(fp);
 
    add_post(currboard, fpath, title, vch.vgamble == '$' ? "[賭神]" : "[投票所]", "系統公告", 0);
    unlink(fpath);
  }
  ...
  VCH *vch;
{
  ...
  { 
      ...
      FILE *fp;
      int count, i;
      char pfpath[64], *slist[MAX_CHOICES];
      vitem_t vlist[MAX_CHOICES];

      if ((count = vote_readvlist(vlist, vch, xo->dir)) <= 0)
	  return;

      for (i = 0; i < count; i++)
	  slist[i] = (char *) &vlist[i];

      sprintf(pfpath, "tmp/vote.%s", cuser.userid);
      if (!(fp = fopen(pfpath, "w")))
	return;

      /* 文章檔頭 */
      fprintf(fp, "%s [賭神] (系統公告)\n", str_author1);
      fprintf(fp, "標題: [公告] 賭盤取消\n時間: %s\n\n", Now());

      /* 文章內容 */
      fprintf(fp, "賭盤 " ANSI_COLOR(1) "%s" ANSI_RESET " 已被取消，賭金將會退回。\n\n", vch->title);
      ...
      *fname = 'G';
	  ...
	  while (read(fd, &vlog, sizeof(VLOG)) == sizeof(VLOG))
	  {
	      ...
	      fprintf(fp, "%-15s買了 %5d 張 %s，退回 %5d 元\n"
		      , vlog.userid, vlog.numvotes, slist[vote_index(vlog.choice)] ,vlog.numvotes * vch->price);
	  }
      }
      close(fd);

      /* 文章檔尾 */
      fprintf(fp, EDIT_BANNER, "[賭神]", "系統公告", "系統公告");
      fclose(fp);

      add_post(currboard, pfpath, "[公告] 賭盤取消", "[賭神]", "系統公告", 0);
      unlink(pfpath);
  }
