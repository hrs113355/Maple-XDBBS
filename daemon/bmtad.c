  ...
  Agent *ap;
{
  ...
  {
    ap->xerro++;
    agent_reply(ap, cc < 0 ? "501 格式錯誤" : "551 we dont accept relayed mail");
  ...
  Agent *ap;
{
  ...
  {
    ap->xerro++;
    agent_reply(ap, "503 請先決定收信人");
    return;
  }

  ap->mode = mode | AM_DATA;
  agent_reply(ap, "354 請開始輸入訊息(換行-.-換行結束)");
}
