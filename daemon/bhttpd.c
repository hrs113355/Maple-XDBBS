...
//  2)水桶名單: 好友:不可發表  壞人:看不見
  ...
  pbad = bshm->pbad + (brd - bshm->bcache);
  
  btype = is_brdgood(uno, bpal) ? 1 : is_brdbad(uno, bpal) ? 2 : 0;
...
//  if ((!over18() || !ulevel) && (brd->battr & BRD_18X))
//      bits = 0;
//TODO  
